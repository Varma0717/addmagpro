        ...
        children: [
          ...
          if (data.team != null) ...[
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Team Levels', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                Text('${data.team!.totalTeam} members', style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
              ],
            ),
            const SizedBox(height: 12),
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemCount: data.team!.levels.length,
                separatorBuilder: (_, __) => const SizedBox(width: 10),
                itemBuilder: (context, index) {
                  final level = data.team!.levels[index];
                  return _LevelStatCard(level: level);
                },
              ),
            ),
            const SizedBox(height: 18),
            const Text('Referral Hierarchy', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
            const SizedBox(height: 8),
            if (data.team!.members.isEmpty)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 8),
                child: Text('No team hierarchy yet', style: TextStyle(color: AppColors.textMuted)),
              )
            else
              ..._buildHierarchy(data.team!.members),
            const SizedBox(height: 18),
          ],
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
